...
- (BOOL) verifyDownload {
    ...
    if(isVerified) {
        if(self.completed){
            //NSLog(@"Already Download Completed...!!");
        } else {
            self.completed = YES;
            self.totalBytesWritten = self.totalBytesExpectedToWrite;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[BaseController sharedInstance] showToastSuccess:@"Your Offline Song Saved Successfully..."];
            });
        }
    }//end if
    /*if(self.completed) {
...
- (NSString *) fullErrorDescription {
    if(self.error) {
        ...
        return [NSString stringWithFormat:@"Offline Songs URL %@ failed because of error: %@ (Code %d)", self.urlString, [self.error localizedDescription], errorCode];
    } else {
        return @"No Error";
    }//end else
}
