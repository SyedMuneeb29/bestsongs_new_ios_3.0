...
#define OBJECTIVECDM_MAX_TIME_OUT 300
...
- (void) resumeAllSuspendedTasks {    
    [session getTasksWithCompletionHandler:^(NSArray *dataTasks, NSArray *uploadTasks, NSArray *downloadTasks) {
        ...
        if(isFound){
//            isFound = NO;
            [[BaseController sharedInstance] showToastSuccess:@"Your Offline Saving has Been Resume..."];
        }
    }];
...
- (void) suspendAllOnGoingDownloadTask {
    [session getTasksWithCompletionHandler:^(NSArray *dataTasks, NSArray *uploadTasks, NSArray *downloadTasks) {
        ...
        if(isFound){
//            isFound = NO;
            [[BaseController sharedInstance] showToastError:@"Your Offline Saving has Been Pause..."];
        }
    }];
}
