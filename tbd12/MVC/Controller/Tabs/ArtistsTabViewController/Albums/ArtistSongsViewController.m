...
@property (nonatomic, strong) ParallaxViewController *headerView;
@end
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@",  _artist.Name];
    ...
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)
        width = 250;
    self.headerView = [ParallaxViewController instantiateFromNib];
    [self.headerView.nameLabel setText:_artist.Name];
    [self.TableView setParallaxHeaderView:self.headerView mode:VGParallaxHeaderModeTopFill height:(screenRect.size.width - width )];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellID = @"ArtistAlbumCell";
...
- (void) retrieveData{
    @try {
        if(!isDataLoaded){
            [self.headerView.poster sd_setImageWithURL:[NSURL URLWithString:self.artist.Poster]
                                            placeholderImage:[UIImage imageNamed:DEFAULTALBUMART]];
            [self hideNoInternet];
            [self showLoading];
        }
        [[BestsongsAPI sharedInstance] fetchArtistAlbums:_artist.ID page:[NSNumber numberWithInteger:currentPage] onSuccess:^(id response) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^ {
                ...
                if(!isDataLoaded){
                    ...
                    /*__weak ArtistSongsViewController *weakSelf = self;
...
- (void) pullToRefreshData{
    @try {
        ...
        [[BestsongsAPI sharedInstance] fetchArtistAlbums:_artist.ID page:[NSNumber numberWithInteger:1] onSuccess:^(id response) {
            NSDictionary *dataDictionary = (NSDictionary *) response;
            NSMutableArray *dataAlbums = [[NSMutableArray alloc] initWithArray:[[BestsongsAPI sharedInstance] getAlbumsArrayFromJSON:dataDictionary[@"albums"]]];
            dispatch_async(dispatch_get_main_queue(), ^ {
                currentPage = 2;
